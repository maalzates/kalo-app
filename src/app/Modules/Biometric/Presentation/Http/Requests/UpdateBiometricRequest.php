...
class UpdateBiometricRequest extends FormRequest
{
    ...
    {
        $this->merge([
            'biometric_id' => $this->route('biometric')->id ?? $this->route('biometric'),
        ]);
    }
    ...
    public function toDTO(): UpdateBiometricDTO
    {
        $biometric = $this->route('biometric');
        $biometricId = $biometric instanceof Biometric ? (string) $biometric->id : (string) $biometric;

        return new UpdateBiometricDTO(
            biometricId: $biometricId,
            weight: $this->input('weight') ? (string) $this->input('weight') : null,
