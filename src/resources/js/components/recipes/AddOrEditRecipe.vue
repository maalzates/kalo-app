<template>
  ...
  >
    <v-card class="bg-grey-lighten-4">
      ...
      <v-container class="pa-4" style="max-width: 800px">
        ...
        <v-card rounded="xl" class="pa-4 mb-4" elevation="1" border="sm">
          <div class="text-subtitle-2 mb-2 ml-1 font-weight-bold text-grey-darken-2">Añadir Ingredientes</div>
          <v-autocomplete
  ...
  ></v-btn>
        ...
        <div v-if="recipeForm.ingredients.length === 0" class="text-center pa-12 text-grey">
          <v-icon size="64" class="mb-2 opacity-20">mdi-silverware-clean</v-icon>
          <div class="text-body-1 opacity-60">Añade ingredientes para ver los macros</div>
        </div>
      </v-container>
    </v-card>
  </v-dialog>
...
const calculateIngCalories = (ing) => {
  if (!ing.amount || !ing.base_amount) return 0;
  // Factor = (Cantidad deseada) / (Cantidad base definida en el ingrediente)
  // Ejemplo Huevo: 2 (unidades) / 1 (unidad base) = factor 2.
  // Ejemplo Arroz: 150 (g) / 100 (g base) = factor 1.5.
  const factor = ing.amount / ing.base_amount;
  return (ing.calories * factor).toFixed(0);
};

const totals = computed(() => {
  return recipeForm.ingredients.reduce((acc, ing) => {
    const amount = ing.amount || 0;
    const baseAmount = ing.base_amount || 1; // Evitar división por cero
    const factor = amount / baseAmount;
    
    acc.calories += ing.calories * factor;
    acc.protein += (ing.protein || 0) * factor;
    acc.carbs += (ing.carbs || 0) * factor;
    acc.fat += (ing.fat || 0) * factor;
    return acc;
...
<style scoped>
/* Estilos para el input de gramaje dentro de la cápsula */
