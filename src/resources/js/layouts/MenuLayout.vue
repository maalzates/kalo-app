<template>
    <template v-if="userStore.isLoggedIn">
        <v-app-bar color="deep-purple" elevation="0" border="b">
            ...
            <v-app-bar-title class="logo"
                >KALO<span class="logo-light">APP</span></v-app-bar-title
            >
            <v-spacer></v-spacer>
            ...
            <div class="d-md-none mr-4" @click="$router.push('/profile')">
                ...
                >
                    <span
                        class="text-caption text-deep-purple font-weight-bold"
                        >{{ userStore.userInitials }}</span
                    >
                </v-avatar>
            </div>
        </v-app-bar>
        ...
        <v-navigation-drawer v-model="drawer" color="deep-purple" theme="dark">
            <v-list nav class="mt-4 main-menu">
                <v-list-item
                    ...
                    to="/"
                />
                <!-- TODO: DELETE INGREDIENTS.VUE AND RECIPES.VUE PARENT COMPONENTS -->
                ...
                <v-list-item class="pa-4">
                    <template v-slot:prepend>
                        <v-avatar color="deep-purple-lighten-4" class="mr-2">
                            <span
                                class="text-body-2 text-deep-purple font-weight-bold"
                                >{{ userStore.userInitials }}</span
                            >
                        </v-avatar>
                    </template>
                    ...
                    <v-list-item-subtitle
                        >Meta:
                        {{ currentMacroCalories }} kcal</v-list-item-subtitle
                    >
    ...
    <v-main>
        <v-container
            fluid
            :class="userStore.isLoggedIn ? 'pa-6' : 'pa-0 fill-height'"
        >
            <slot />
        </v-container>
    </v-main>
